...
<style>
body {font-family: "lucida grande","Segoe UI",Arial, sans-serif; font-size: 14px;padding:1em;margin:0;}
...
label { display:block; font-size:21px; color:#555;}
#file_drop_target { padding:12px 0; border: 4px dashed #ccc;font-size:12px;color:#ccc;
...
#breadcrumb { padding-top:4px; font-size:25px; color:#aaa;display:inline-block;float:left;}
...
td.first {font-size:24px;white-space: normal;}
...
<?php endif; ?>
	...
	function renderFileRow(data) {
		...
		var extension2 =  data.path.slice(index+1);
		...
		}else if( extension2 == 'jpg' || extension2 == 'jpeg' || extension2 == 'png'){
			//console.log('not is dir -data.path',data.path);
			// let ttt = data.path.split('.');
			// let extension = ttt[1];
			$link = $('<img class="name2" />')
			.attr('src', data.is_dir ? '#' + encodeURIComponent(data.path) : './' + data.path)
			.css("background-image", "");// .style.backgroundImage = '';
		}else if (extension2 == 'mp4'){
			$link = $('<video width="400" height="400" controls="controls" >')
			.append( $('<source />')
			.attr('src', './' + data.path)
			.attr('type', 'video/mp4'));
			// .text('not play video on yours device');
		}else{
			$link = $('<a class="name" />')
			.attr('href', data.is_dir ? '#' + encodeURIComponent(data.path) : './' + data.path)
			.text(data.name);
		...
		if(ww2 > 400){
			tdStr = '<td />';
			$('th').removeClass('desktop');
		}else{
			tdStr = '<td class="desktop"/>';
		...
		var $html = $('<tr />')
			...
			.append( $(tdStr).attr('data-sort',data.is_dir ? -1 : data.size)
				.html($('<span class="size" />').text(formatFileSize(data.size))) )
			.append( $(tdStr).attr('data-sort',data.mtime).text(formatTimestamp(data.mtime)) )
			.append( $(tdStr).text(perms.join('+')) )
			.append( $(tdStr).append($dl_link).append( data.is_deleteable ? $delete_link : '') )
		return $html;
	}
...
<table id="table"><thead><tr>
	...
	<th class='desktop'>Modified</th>
	<th class='desktop'>Permissions</th>
	<th class='desktop'>Actions</th>
</tr></thead><tbody id="list">

</tbody></table>
<footer>simple php filemanager by <a href="https://monds.tech">monds.tech</a></footer>
</body></html>
